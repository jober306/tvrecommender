...
public class MllibWrapper {
	...
	public static MatrixFactorizationModel train(RecsysTVDataSet dataSet, int matrixRank, int numIter){
		JavaRDD<Rating> ratings = dataSet.convertToMLlibRatings();
		return ALS.train(JavaRDD.toRDD(ratings), matrixRank, numIter);
	}
	
	public static void evaluateModel(MatrixFactorizationModel model, RecsysTVDataSet dataSet){
		JavaRDD<Rating> ratings = dataSet.convertToMLlibRatings();
		...
		JavaPairRDD<Tuple2<Integer,Integer>, Double> predictions = JavaPairRDD.fromJavaRDD(model.predict(JavaRDD.toRDD(userProducts)).toJavaRDD()
				.map(rating -> new Tuple2<>(new Tuple2<>(rating.user(), rating.product()), rating.rating())));
		
		JavaRDD<Tuple2<Double,Double>> ratesAndPreds = JavaPairRDD.fromJavaRDD(ratings
				.map(rating -> new Tuple2<>(new Tuple2<>(rating.user(), rating.product()), rating.rating()))).join(predictions).values();
		double MSE = JavaDoubleRDD.fromRDD(ratesAndPreds.map( rateAndPred -> {
			Double err = rateAndPred._1() - rateAndPred._2();
			return Object.class.cast(err * err);}).rdd()).mean();
	...
	public static void main(String[] args){
		...
		MatrixFactorizationModel model = MllibWrapper.train(dataSet, 10, 10);
